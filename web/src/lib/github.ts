...
export async function createManifestPR(
  owner: string,
  repo: string,
  ...
  const branchName = "add-dotfiles-manifest";

  // 1. Get the SHA of the default branch
  const { data: ref } = await apiFetch<GitRef>(
  ...
  try {
    ...
    if (e instanceof GitHubApiError && e.status === 422) {
      // Branch already exists — force-update it to the latest base
      await fetch(`${API_BASE}/repos/${owner}/${repo}/git/refs/heads/${branchName}`, {
        method: "PATCH",
        headers: {
          Accept: "application/vnd.github+json",
          Authorization: `Bearer ${token}`,
  ...
  // 3. Create (or update) the manifest file on the new branch
  const contentBase64 = btoa(unescape(encodeURIComponent(manifestJson)));

  // Check if file already exists to get its SHA
  let existingSha: string | undefined;
  try {
    const { data: existing } = await apiFetch<{ sha: string }>(
      `/repos/${owner}/${repo}/contents/.dotfiles-manifest.json?ref=${branchName}`,
      token,
    );
  ...
  await apiPut(
    `/repos/${owner}/${repo}/contents/.dotfiles-manifest.json`,
    {
      message: "Add dotfiles-installer manifest",
  ...
  const pr = await apiPost<CreatedPR>(
    `/repos/${owner}/${repo}/pulls`,
    {
      title: "Add dotfiles-installer manifest",
      body:
        ...
        "```bash\n" +
        `curl -fsSL https://raw.githubusercontent.com/jeromesch/dotfiles-installer/main/installer/install.sh | bash -s -- ${owner}/${repo}\n` +
        "```",
      head: branchName,
      base: defaultBranch,
    },
